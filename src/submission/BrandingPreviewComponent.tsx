    ...
    return (
        <div className="cbBrandingPreview">
            <ThumbnailSelectionComponent
                ...
                time={displayedThumbnail.type === ThumbnailType.SpecifiedTime ? (displayedThumbnail as any).timestamp : undefined}
                hideTime={true}
                larger={true}
            ></ThumbnailSelectionComponent>
    ...
    if (submissions.thumbnails.some((t) => t.votes >= 0)) {
        ...
        if (!best.original) {
            return thumbnails.find((t) => t.type === ThumbnailType.SpecifiedTime
                && (t as any).timestamp === (best as any).timestamp) ?? thumbnails[0];
        } else {
            return thumbnails[0];
        }
    } else {
